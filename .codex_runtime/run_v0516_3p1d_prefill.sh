...
# CP round-robin sparse prefill uses rank-local query metadata. Set to 0 for
# the production rollback to the paged attention path.
...
dist_ports=(8326 8327 8328)
bootstrap_ports=(18956 18957 18958)
gpu_sets=(2,3 4,5 6,7)
pids=()

for index in 0 1 2; do
  log_file="${ARTIFACT_DIR}/prefill-p${index}.log"
  CUDA_VISIBLE_DEVICES="${gpu_sets[index]}" \
  SGLANG_TORCH_PROFILER_DIR="${ARTIFACT_DIR}/profile-p${index}" \
  sglang serve \
    ...
    --host 6.200.20.13 \
    --port "${ports[index]}" \
    --dist-init-addr "6.200.20.13:${dist_ports[index]}" \
