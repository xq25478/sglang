...
class TestCpCacheLayerSplitDeepSeekV4PoolInternals(CustomTestCase):
    @staticmethod
    def _make_sparse_remap_pool():
        fake = object.__new__(CpCacheLayerSplitDeepSeekV4TokenToKVPool)
        fake._swa_remapped_layer_id = 0
        fake._extra_staging_layer_id = 1
        fake._extra_page_table_remap_layer_id = 1
        fake._extra_page_table_selected_pages = torch.tensor([1, 3])
        fake._extra_page_table_page_size = 4
        fake._extra_page_table_max_pages = 4
        fake._batch_active_pages = {
            "swa": SimpleNamespace(selected_pages=torch.tensor([1, 3])),
            "extra_c128": SimpleNamespace(selected_pages=torch.tensor([1, 3])),
        }
        fake.swa_kv_pool = SimpleNamespace(size=12, page_size=4)
        fake.c4_kv_pool = SimpleNamespace(size=12, page_size=4)
        fake.c128_kv_pool = SimpleNamespace(size=12, page_size=4)
        fake.layer_mapping = [
            SimpleNamespace(compress_ratio=0),
            SimpleNamespace(compress_ratio=4),
            SimpleNamespace(compress_ratio=128),
        ]
        fake._stage_start = 0
        fake._stage_end = 3
        fake._c4_owner_local_by_layer = {1: 0}
        fake._c128_owner_local_by_layer = {2: 0}
        return fake

    def test_sparse_prefill_token_ids_are_remapped_to_staging(self):
        fake = self._make_sparse_remap_pool()
        eager_remap = torch.compiler.disable(staging.remap_indices_to_staging)
        token_ids = torch.tensor([4, 5, 12, 15], dtype=torch.int32)

        with patch(
            "sglang.srt.mem_cache.cp_cache_layer_split.deepseek_v4_pool."
            "remap_indices_to_staging",
            eager_remap,
        ):
            self.assertEqual(
                fake.remap_sparse_prefill_swa_token_ids(0, token_ids).tolist(),
                [0, 1, 4, 7],
            )
            self.assertEqual(
                fake.remap_sparse_prefill_extra_token_ids(1, token_ids).tolist(),
                [0, 1, 4, 7],
            )

            fake._extra_staging_layer_id = 2
            self.assertEqual(
                fake.remap_sparse_prefill_extra_token_ids(2, token_ids).tolist(),
                [0, 1, 4, 7],
            )

    def test_sparse_prefill_masked_extra_slots_remain_in_bounds(self):
        fake = self._make_sparse_remap_pool()
        eager_remap = torch.compiler.disable(staging.remap_indices_to_staging)
        ...
        with patch(
            ...
            remapped = fake.remap_sparse_prefill_extra_token_ids(
                1, torch.tensor([0, 4, 12], dtype=torch.int32)
            )

        self.assertEqual(remapped.tolist(), [0, 0, 4])
    ...
    def test_non_owner_extra_page_size_uses_compressed_family_pool(self):
        fake = object.__new__(CpCacheLayerSplitDeepSeekV4TokenToKVPool)
        fake._stage_start = 0
        fake._stage_end = 2
        fake.layer_mapping = [
        ...
        fake._c4_owner_local_by_layer = {0: 0}
        fake._c128_owner_local_by_layer = {1: 0}
        fake.c4_kv_pool = SimpleNamespace(page_size=4)
        fake.c128_kv_pool = SimpleNamespace(page_size=128)

        self.assertEqual(fake.get_extra_key_page_size(0), 4)
        self.assertEqual(fake.get_extra_key_page_size(1), 128)
