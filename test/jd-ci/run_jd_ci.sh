...
CI_RUN_ID="${TIMESTAMP}_${BRANCH_NAME_FOR_DOCKER}_${COMMIT_ID}"
...
CI_LOGS_DIR="${CI_ARTIFACT_ROOT}/ci_logs/${CI_RUN_ID}"
MAIN_PIPELINE_LOG="${CI_LOGS_DIR}/main_pipeline.log"
MSTORE_PIPELINE_LOG="${CI_LOGS_DIR}/mooncake_store_pipeline.log"
CI_TMP_DIR="${CI_ARTIFACT_ROOT}/tmp/"
CONTAINER_TMP_RUNTIME_DIR="${CI_TMP_DIR%/}"
CONTAINER_CUDA_CACHE_DIR="${CONTAINER_TMP_RUNTIME_DIR}/cuda_cache"
CONTAINER_SGL_KERNEL_TMP_DIR="${CONTAINER_TMP_RUNTIME_DIR}/sgl-kernel"
CONTAINER_MOONCAKE_TMP_DIR="${CONTAINER_TMP_RUNTIME_DIR}/mooncake"
CONTAINER_UV_CACHE_DIR="${CONTAINER_TMP_RUNTIME_DIR}/uv-cache"
CONTAINER_PIP_CACHE_DIR="${CONTAINER_TMP_RUNTIME_DIR}/pip-cache"
CONTAINER_XDG_CACHE_HOME="${CONTAINER_TMP_RUNTIME_DIR}/xdg-cache"
CONTAINER_TORCH_EXTENSIONS_DIR="${CONTAINER_TMP_RUNTIME_DIR}/torch-extensions"
CONTAINER_TRITON_CACHE_DIR="${CONTAINER_TMP_RUNTIME_DIR}/triton-cache"
CONTAINER_TORCHINDUCTOR_CACHE_DIR="${CONTAINER_TMP_RUNTIME_DIR}/torchinductor-cache"
CI_TMP_CLEANUP_TIMEOUT_SEC="${CI_TMP_CLEANUP_TIMEOUT_SEC:-300}"
...
rm_ci_tmp_dir() {
    run_with_timeout "${CI_TMP_CLEANUP_TIMEOUT_SEC}" rm -rf "${CI_TMP_DIR}" 2>/dev/null || true
}

cleanup_ci_tmp_dir() {
    local label="${1:-收尾清理}"
    if [[ "${CI_TMP_DIR:-}" != "${CI_ARTIFACT_ROOT}/tmp/" ]]; then
        echo "[SGLang CI] WARN: 拒绝清理非预期临时目录: ${CI_TMP_DIR:-<unset>}"
        return
    fi
    rm_ci_tmp_dir
    if [[ -d "${CI_TMP_DIR}" ]]; then
        echo "[SGLang CI] 普通删除未清空临时目录，尝试容器内 root 兜底清理: ${CI_TMP_DIR}"
        run_with_timeout "${DOCKER_CLEANUP_TIMEOUT_SEC}" docker run --rm \
            --platform linux/amd64 \
            -v "${CI_TMP_DIR}:/ci-tmp" \
            --entrypoint /bin/bash \
            "${BASE_IMAGE}" \
            -c "shopt -s dotglob nullglob; rm -rf /ci-tmp/*" >/dev/null 2>&1 || true
        rm_ci_tmp_dir
    fi
    if [[ -d "${CI_TMP_DIR}" ]]; then
        echo "[SGLang CI] WARN: ${label}临时目录未完全删除: ${CI_TMP_DIR}"
    else
        echo "[SGLang CI] ${label}临时目录完成: ${CI_TMP_DIR}"
    fi
}
...
cleanup_temp_artifact_dir() {
    local label="${1:-收尾清理}"
    if [[ -z "${JD_CI_TEMP_ARTIFACT_ROOT:-}" ]]; then
        return
    fi
    if [[ "${JD_CI_TEMP_ARTIFACT_ROOT}" != "${CI_ARTIFACT_ROOT}/tmp_artifacts/"* ]]; then
        echo "[SGLang CI] WARN: 拒绝清理非预期临时产物目录: ${JD_CI_TEMP_ARTIFACT_ROOT}"
        return
    fi
    echo "[SGLang CI] ${label}临时编译产物目录: ${JD_CI_TEMP_ARTIFACT_ROOT}"
    run_with_timeout "${CI_TMP_CLEANUP_TIMEOUT_SEC}" rm -rf "${JD_CI_TEMP_ARTIFACT_ROOT}" 2>/dev/null || true
    if [[ -d "${JD_CI_TEMP_ARTIFACT_ROOT}" ]]; then
        echo "[SGLang CI] WARN: ${label}临时编译产物目录未完全删除: ${JD_CI_TEMP_ARTIFACT_ROOT}"
    else
        echo "[SGLang CI] ${label}临时编译产物目录完成: ${JD_CI_TEMP_ARTIFACT_ROOT}"
    fi
}
...
chmod 1777 "${CI_TMP_DIR}" 2>/dev/null || true
...
echo "[SGLang CI] CI_TMP_DIR=${CI_TMP_DIR}"
echo "[SGLang CI] CONTAINER_TMP_RUNTIME_DIR=${CONTAINER_TMP_RUNTIME_DIR}"
echo "[SGLang CI] CONTAINER_CUDA_CACHE_DIR=${CONTAINER_CUDA_CACHE_DIR}"

# 构建镜像信息
IMAGE_ENTRYPOINT_SCRIPT="${CI_TMP_DIR}/entrypoint.sh"
pushd "${SOURCE_PATH}" > /dev/null
...
run_docker_attached docker run \
    ...
    -e CI_TMP_DIR="${CONTAINER_TMP_RUNTIME_DIR}" \
    -e TMPDIR="${CONTAINER_TMP_RUNTIME_DIR}" \
    -e TMP="${CONTAINER_TMP_RUNTIME_DIR}" \
    -e TEMP="${CONTAINER_TMP_RUNTIME_DIR}" \
    -e CUDA_CACHE_PATH="${CONTAINER_CUDA_CACHE_DIR}" \
    -e SGL_KERNEL_BUILD_TMPDIR="${CONTAINER_SGL_KERNEL_TMP_DIR}" \
    -e MOONCAKE_TMP_DIR="${CONTAINER_MOONCAKE_TMP_DIR}" \
    ...
    -e TORCHINDUCTOR_CACHE_DIR="${CONTAINER_TORCHINDUCTOR_CACHE_DIR}" \
    ...
    -c "
        set -euo pipefail
        trap 'rm -rf "${CONTAINER_TMP_RUNTIME_DIR}"/* "${CONTAINER_TMP_RUNTIME_DIR}"/.[!.]* "${CONTAINER_TMP_RUNTIME_DIR}"/..?* /tmp/* /tmp/.[!.]* /tmp/..?* 2>/dev/null || true' EXIT
        echo '[JD CI] CI 容器启动成功!'
        echo '[JD CI] CI 工作路径:' \$(pwd)
        mkdir -p \
            '${CONTAINER_TMP_RUNTIME_DIR}' \
            '${CONTAINER_CUDA_CACHE_DIR}' \
            '${CONTAINER_SGL_KERNEL_TMP_DIR}' \
            '${CONTAINER_MOONCAKE_TMP_DIR}' \
            '${CONTAINER_UV_CACHE_DIR}' \
            '${CONTAINER_PIP_CACHE_DIR}' \
            '${CONTAINER_XDG_CACHE_HOME}' \
            '${CONTAINER_TORCH_EXTENSIONS_DIR}' \
            '${CONTAINER_TRITON_CACHE_DIR}' \
            '${CONTAINER_TORCHINDUCTOR_CACHE_DIR}'
        chmod 1777 '${CONTAINER_TMP_RUNTIME_DIR}' 2>/dev/null || true
        ...
        if [[ '${JD_CI_SKIP_MOONCAKE_BUILD}' == '1' ]]; then
            ...
            set -o pipefail
            mkdir -p ${CI_TMP_DIR}/mkcompile
            bash ${SOURCE_PATH}/test/jd-ci/env/build_mooncake.sh ${CI_TMP_DIR}/mkcompile "te" \
                | tee ${CI_LOGS_DIR}/build_mooncake.log
            set +o pipefail
        fi
        ...
        if [[ '${JD_CI_SKIP_SGL_KERNEL_BUILD}' == '1' ]]; then
            ...
            bash ${SOURCE_PATH}/test/jd-ci/env/build_sgl_kernel.sh \
                ...
                '${CI_WORK_DIR}' \
                '/wheels' \
                '/sgl-workspace/sglang/sgl-kernel' 2>&1 | tee ${CI_LOGS_DIR}/build_sgl_kernel.log
            set +o pipefail
        fi
        ...
        if [[ '${RUN_CI_TESTS}' == '1' ]]; then
            ...
            bash '${SOURCE_PATH}/test/jd-ci/pipeline/run_cpu_mock_regression.sh' \
                '${SOURCE_PATH}' '${JD_CI_BASE_REF}' '${CI_LOGS_DIR}' \
...
run_docker_attached docker run \
    ...
    -c "
        ...
        if [[ '${JD_CI_SKIP_MOONCAKE_BUILD}' == '1' ]]; then
            ...
            set -o pipefail
            mkdir -p ${CI_TMP_DIR}/mkcompile
            bash ${SOURCE_PATH}/test/jd-ci/env/build_mooncake.sh ${CI_TMP_DIR}/mkcompile store \
                | tee ${CI_LOGS_DIR}/build_mooncake_store.log
            set +o pipefail
        fi
...
if [ -f "${CI_LOGS_DIR}/jd_ci_report.md" ]; then
    echo "[SGLang CI] 汇总报告 Markdown: ${CI_LOGS_DIR}/jd_ci_report.md"
fi
if [ -f "${CI_LOGS_DIR}/jd_ci_report.json" ]; then
    echo "[SGLang CI] 汇总报告 JSON: ${CI_LOGS_DIR}/jd_ci_report.json"
fi
