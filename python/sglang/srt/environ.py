...
class Envs:
    ...
    # P0 correctness guard for #31023. A device-wide barrier immediately before
    # compact target-verify graph replay suppresses the observed asynchronous
    # illegal-memory-access race. Keep this isolated to DSpark compact graphs and
    # allow performance experiments to opt out while the exact stream dependency
    # is being localized upstream.
    SGLANG_DSPARK_COMPACT_CUDA_GRAPH_SYNC = EnvBool(True)
    SGLANG_DSPARK_PD_TARGET_LAYER_IDS = EnvTuple(tuple())
    SGLANG_DSPARK_PD_HIDDEN_POOL_TOKENS = EnvInt(None)
    SGLANG_DSPARK_PD_HIDDEN_RECV_POOL_TOKENS = EnvInt(None)
    ...
    SGLANG_OPT_USE_CP_MULTI_STREAM_OVERLAP = EnvBool(False)
