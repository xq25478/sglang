...
class CpCacheLayerSplitDeepSeekV4TokenToKVPool(
    ...
    def __init__(
        ...
        **kwargs,
        ...
        disable_async_broadcast = (
            envs.SGLANG_CP_CACHE_LAYER_SPLIT_DISABLE_ASYNC_BROADCAST.get()
        )
        self._broadcast_slots = BroadcastSlots(
            ("swa", "extra", "indexer"),
            async_enabled=not disable_async_broadcast,
        )
        logger.info(
            "DSV4 Cache LayerSplit broadcast mode: %s",
            "synchronous" if disable_async_broadcast else "asynchronous",
        )
    ...
    def remap_sparse_prefill_swa_token_ids(
        self, layer_id: int, token_ids: torch.Tensor
    ) -> torch.Tensor:
        """Map sparse-prefill SWA slots into the compact staging buffer."""
        if self._swa_remapped_layer_id != layer_id:
            return token_ids

        swa_cache = self._batch_active_pages["swa"]
        assert swa_cache.selected_pages is not None
        return remap_indices_to_staging(
            token_ids,
            swa_cache.selected_pages,
            self.swa_kv_pool.page_size,
            self._pool_num_pages(self.swa_kv_pool),
        )
    ...
    def get_extra_key_page_size(self, layer_id: int) -> int:
        """Return the compressed-family page size on owner and non-owner ranks."""
        owner_item = self._owner_compress_layer_item(layer_id)
        assert owner_item.compress_kv_pool is not None
        return owner_item.compress_kv_pool.page_size
    ...
    def remap_sparse_prefill_extra_token_ids(
        self, layer_id: int, token_ids: torch.Tensor
    ) -> torch.Tensor:
        """Map sparse-prefill C4/C128 slots into compact staging buffers."""
        if self._extra_staging_layer_id != layer_id:
            return token_ids
        ...
        assert item is not None
        if item.compress_ratio == 4:
            assert self._extra_page_table_remap_layer_id == layer_id
            assert self._extra_page_table_selected_pages is not None
            assert self._extra_page_table_page_size is not None
            assert self._extra_page_table_max_pages is not None
            selected_pages = self._extra_page_table_selected_pages
            page_size = self._extra_page_table_page_size
            max_pages = self._extra_page_table_max_pages
        else:
            assert item.compress_ratio == 128
            c128_cache = self._batch_active_pages["extra_c128"]
            assert c128_cache.selected_pages is not None
            owner_item = self._owner_compress_layer_item(layer_id)
            assert owner_item.compress_kv_pool is not None
            pool = owner_item.compress_kv_pool
            selected_pages = c128_cache.selected_pages
        ...
        # Sparse-prefill scaffolding represents masked compressed entries with
        # slot 0. If page 0 is not active, keep those unused rows in bounds;
        # the sparse attention lengths prevent them from being consumed.
        return remap_indices_to_staging(
            token_ids, selected_pages, page_size, max_pages
        ).clamp_min(0)
