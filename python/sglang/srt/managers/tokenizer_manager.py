        ...
        if should_run_mm_processor:
            ...
            if (
                ...
                if mm_inputs is None:
                    ...
                    async with self._mm_semaphore:
                        try:
                            mm_inputs = await asyncio.wait_for(
                                ...
                                timeout=self.server_args.mm_per_request_timeout,
                            )
                        except asyncio.TimeoutError:
                            raise ValueError(
                                f"Multimodal preprocessing timed out after "
                                f"{self.server_args.mm_per_request_timeout}s. "
                                f"Reduce image/video size or increase "
                                f"--mm-per-request-timeout."
                            )
            elif (
