...
def build_draft_tp_worker(
    *,
    ...
    """Build the shared draft worker using the v0.5.15 ServerArgs API.

    Upstream main uses ``ServerArgs.override`` here.  That API is newer than
    JD-v0.5.15, so this backport applies the same resolved values directly and
    restores the scheduler-global ServerArgs after draft-worker construction.
    """
    ...
    draft_server_args.speculative_draft_attention_backend = draft_backend
