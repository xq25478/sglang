    ...
    if not any_sampling:
        ...
        def sampler(step_logits: torch.Tensor, step_idx: int) -> torch.Tensor:
            if fast_sampling:
                ...
                # torch.multinomial rejects all-NaN rows produced by
                # degenerate logits. Keep the request alive with the same
                # deterministic token-0 fallback as the fast Triton sampler.
