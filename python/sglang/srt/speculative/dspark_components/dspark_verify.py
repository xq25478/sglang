...
def verify_logits_adjustments_are_noop(
    sampling_info, grammar_mask: Optional[DFlashGrammarMask] = None
) -> bool:
...
def verify_accept_can_be_folded(
    sampling_info, grammar_mask: Optional[DFlashGrammarMask] = None
) -> bool:
    """Return whether the greedy-only CUDA-graph accept epilogue is exact.

    Sampling requests must use the eager rejection-sampling path because the
    folded epilogue calls ``accept_greedy_triton``.
    """
    if sampling_info is not None and not sampling_info.is_all_greedy:
        return False
    return verify_logits_adjustments_are_noop(sampling_info, grammar_mask)
...
class TargetVerifyExecutor:
    ...
    def accept_and_finalize(
        ...
        *,
        ...
        grammar_verify_lens: Optional[torch.Tensor] = None,
    ) -> AcceptOuts:
        ...
        correct_len, bonus, cap_trim_lens = accept_draft_tokens(
            ...
            grammar_verify_lens=grammar_verify_lens,
        )
        if self._simulate_acc_len > 0 and grammar_verify_lens is None:
...
def accept_draft_tokens(
    *,
    ...
    if grammar_verify_lens is not None:
        cutoff_verify_lens = (
            grammar_verify_lens
            if cutoff_verify_lens is None
            else torch.minimum(cutoff_verify_lens, grammar_verify_lens)
        )
