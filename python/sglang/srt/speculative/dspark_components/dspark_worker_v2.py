...
from sglang.srt.speculative.dflash_utils import generate_dflash_chain_grammar_mask
...
class DSparkWorkerV2(BaseSpecWorker):
    ...
    def get_pd_hidden_transfer_window_size(self) -> Optional[int]:
        """Return the target-hidden suffix needed to seed the draft KV cache.

        The DeepSeek-V4 DSpark draft stores attention KV in an SWA ring. Hidden
        rows older than that ring window are overwritten before the first
        decode step, so transferring them across P/D only wastes memory and
        bandwidth. Other DSpark draft architectures keep the conservative
        full-context behavior until their KV lifetime is proven equivalent.
        """
        if not self._draft_is_moe:
            return None
        window_size = getattr(self.draft_model_runner.model_config, "window_size", None)
    ...
    def _forward_decode(
        ...
        if batch.has_grammar:
            grammar_mask = generate_dflash_chain_grammar_mask(
                reqs=batch.reqs,
                verify_input=draft_input,
                verify_tokens=verify_ids_2d,
        ...
        fold_eligible = (
            ...
            and proposal.folded
            and verify_accept_can_be_folded(sampling_info, grammar_mask)
        ...
        accept = self._verify_executor.accept_and_finalize(
            ...
            draft_tokens=draft_tokens,
            grammar_verify_lens=(
                None if grammar_mask is None else grammar_mask.verify_lens
            ),
        )
        if on_publish is not None:
        ...
        effective_verify_lens = (
            layout.verify_lens.to(torch.int32) if layout is not None else None
        )
        if grammar_mask is not None:
            grammar_verify_lens = grammar_mask.verify_lens.to(torch.int32)
            effective_verify_lens = (
                grammar_verify_lens
                if effective_verify_lens is None
                else torch.minimum(effective_verify_lens, grammar_verify_lens)
            )
        return GenerationBatchResult(
            logits_output=logits_output,
            ...
            cap_lens=effective_verify_lens,
