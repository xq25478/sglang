...
class DecodeCudaGraphRunner(BaseCudaGraphRunner):
    ...
    def can_run_graph(self, forward_batch: ForwardBatch):
        ...
        if self.require_mlp_tp_gather:
            # The synchronized values are raw per-rank request counts on
            # decode-family rounds. This also avoids treating DSpark's
            # confidence-selected verify width as a request batch dimension.
    ...
    def _run_capture_tail_hooks(self, out, forward_batch, num_tokens: int) -> None:
    ...
    def capture_one_shape(
        ...
        with forward_context(ForwardContext(attn_backend=attn_backend)):
            ...
            def run_once():
                ...
                self._run_capture_tail_hooks(out, forward_batch, num_tokens)
                return out
    ...
    def _needs_dspark_compact_replay_sync(
        self, forward_batch: ForwardBatch
    ) -> bool:
        """Return whether the #31023 correctness barrier applies.

        The residual DSpark compact-graph failure is timing-sensitive and can
        also occur on a fresh service's first workload. A device synchronization
        immediately before replay is the only currently validated suppressor.
        Keep the workaround restricted to DSpark compact target verification so
        other speculative algorithms and ordinary decode graphs are unaffected.
        """
        return (
            envs.SGLANG_DSPARK_COMPACT_CUDA_GRAPH_SYNC.get()
    ...
    def execute(
        ...
        with timer_ctx, self.backend.replay_session():
            ...
            if self._needs_dspark_compact_replay_sync(forward_batch):
                # Temporary P0 correctness barrier for #31023. Replace this with
                # the smallest producer-event / replay-stream dependency once the
                # stale metadata or workspace owner is conclusively localized.
